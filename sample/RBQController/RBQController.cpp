...
#include "/home/rainbow/Desktop/UIUK_QUAD_NEW/src/Hubodog5/src/RBSharedMemory.h"
...
public:
    ...
    virtual bool initialize(SimpleControllerIO* io) {
        ...
        shm_opened = false;
        int shmFD;
        shmFD = shm_open(RBCORE_SHM_NAME_DATA, O_CREAT|O_RDWR, 0666);
        if(shmFD == -1){
            io->os() << "Fail to open core shared memory [SharedData]";
        }else{
            if(ftruncate(shmFD, sizeof(RBCORE_SHM)) == -1){
                io->os() << "Fail to truncate core shared memory [SharedData]";
            }else{
                sharedData = (pRBCORE_SHM)mmap(0, sizeof(RBCORE_SHM), PROT_WRITE, MAP_SHARED, shmFD, 0);
                if(sharedData == (void*)-1){
                    io->os() << "Fail to mapping core shared memory [SharedData]";
                }
            }
        }
        io->os() << "Core shared memory creation = OK [SharedData]";
        // =========================================================================
...
//        pHRFT = forceSensor[0];
    ...
    void getPosRef(){
        for(int i=0; i<NO_OF_JOINTS; i++){
            Pos_ref[i] = sharedData->joint_info_ref[i];
        }
    }

    void getVelRef(){
        for(int i=0; i<NO_OF_JOINTS; i++){
            Vel_ref[i] = sharedData->joint_info_ref[i+12];
        }
    }

    void getTorqueRef(){
        for(int i=0; i<NO_OF_JOINTS ; i++){
            Tau_ref[i] = sharedData->choreonoid_refJointTorque[i];
        }
    }

    void getGains(){
        for(int i=0; i<NO_OF_JOINTS ; i++){
            pGain[i] = sharedData->Joint_pGain[i];
            dGain[i] = sharedData->Joint_dGain[i];
        }
    }

    void setPosNow(){
        for(int i=0; i<NO_OF_JOINTS ; i++){
            sharedData->choreonoid_JointPosNow[i] = q[i];
        }
    }
    void setVelNow(){
        for(int i=0; i<NO_OF_JOINTS ; i++){
            sharedData->choreonoid_JointVelNow[i] = dq[i];
        }
    }
...
//        io->os() << P << endl;
        ...
        for(int i=0; i<3 ; i++){
            sharedData->choreonoid_acc[i] = Acc[i];
            sharedData->choreonoid_gyro[i] = Gyro[i];
            sharedData->choreonoid_W_body_pos[i] = P[i];
        }

        sharedData->choreonoid_quat[0] = Qt.w();
        sharedData->choreonoid_quat[1] = Qt.x();
        sharedData->choreonoid_quat[2] = Qt.y();
        sharedData->choreonoid_quat[3] = Qt.z();
        ...
        for(int i=0; i<body->numJoints(); i++){
            Link* joint = body->joint(i);
            joint->u() = LimitFunction(jointInput[i], 180.0, -180.0);
        }
        ...
        for(int i=0 ; i<3; i++){
            sharedData->chorednoid_ReactionF_HR[i] = HRFT_global[i];
            sharedData->chorednoid_ReactionF_HL[i] = HLFT_global[i];
            sharedData->chorednoid_ReactionF_FR[i] = FRFT_global[i];
            sharedData->chorednoid_ReactionF_FL[i] = FLFT_global[i];
        ...
        if(cnt%2==0 && !(sharedData == (void*)-1)) // 500Hz
        {
            sharedData->TimeSyncFlag = 1;
        }
        cnt++;
        return true;
    }
